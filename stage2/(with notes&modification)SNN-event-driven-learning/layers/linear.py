...
class LinearLayer(nn.Linear):
    ...
    def forward(self, x, labels=None):
        if glv.init_flag:
            glv.init_flag = False
            # 调用initialize，调节输入大小
            # 这里x是脉冲形式，这里计算完了就return了，和下面有区分
            x = initialize(self, x)
            glv.init_flag = True
            return x
...
class LinearFunc(torch.autograd.Function):
    ...
    def forward(ctx, inputs, weight, norm_weight, norm_bias, config, labels):
        # config = (theta_m, theta_s, theta_grad, self.threshold)
        ...
        ctx.is_out_layer = labels != None # 标记是否为输出层
