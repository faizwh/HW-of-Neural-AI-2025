...
class learningStat():
    ...
    def reset(self):
        ...
        >>> trainingStat.reset()
        '''
        # reset后，minloss,maxAccuracy,lossLog,accuracyLog,bestLoss,bestAccuracy这些全都保留
        # 所以下面LearningStats大类的update函数里，小的learningstat先update后reset并没有什么问题
        self.lossSum = 0
        self.correctSamples = 0
        self.numSamples = 0
    ...
    def update(self):
        ...
        if self.maxAccuracy is None:
            ...
            if currentAccuracy > self.maxAccuracy:
                self.maxAccuracy = currentAccuracy
                # 还额外记录一个bestAccuracy的标识符，要和maxAccuracy区分开来
    ...
    def displayString(self):
        ...
        if loss is None:    # no stats available
            ...
            if minloss is None: # accuracy and minloss stats is not available
                # %-11.5g：- 表示左对齐；11 表示占用 11 个字符宽度（不够长就补空格）；
                # 5 表示保留 5 位有效数字。整体意为 “左对齐，占 11 个字符，显示 5 位有效数字的数值
                # g表示自动选择科学计数法或浮点数形式显示，保留指定的有效数字
                # 这就是为什么输出时会有很多空隔
...
class learningStats():
    ...
    .. code-block:: python
        ...
        for epoch in range(100):
            ...
            for i in testingLoop
                ...
                stats.print(epoch, i)
            stats.training.update()
    这是一个集成了许多learningStat类型变量的大类
    '''

    def __init__(self):
        self.linesPrinted = 0 # 神必记录变量，大概本来是打算写到loss.txt一类记录当中的
    ...
    def print(self, epoch, iter=None, timeElapsed=None, header=None, footer=None):
        ...
        # timeElapsed 过去了多少时间； 具体这个量是怎么统计的呢，还得看代码
    ...
    def plot(self, figures=(1, 2), saveFig=False, path=''):
        ...
        然而实际上似乎只是使用了tensorboard，而没有生成这里plot函数该产生的png图像
        '''
    ...
    def load(self, filename='', numEpoch=None, modulo=1):
        ...
        .. code-block:: python
            ...
            # save stats
            stats.load(epoch=10)
            感觉好像不太能对得上啊，总之这段代码没有用上，根据代码内容来看，似乎是防止运行中断时，从上次的运行log里继续训练与记录
